...
export default class ClassList extends Component {
	...
	componentWillMount(){
		...
		for (let cid in this.props.classes){
			// reads only once 
			fire.database().ref('/classes/' + cid).once('value', (snap) => {
	...
	render(){
		return(
			...
			<div className="classes-wrapper">
				<ListGroup>
				    <ListGroupItem  id="header"><h3>Your Classes</h3></ListGroupItem>
				    {
				    	this.state.classes.map((c, i) => {
				    		return (
				    			<Link key={i} to={'/class/' + c.id}><ListGroupItem action>{c.name}</ListGroupItem></Link>
				    		);
				...
				<div className="add-class-btn">
					<Link to='/create-class'><Button color="primary"> Add Class </Button></Link>
				</div>
